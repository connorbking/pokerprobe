      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-gray-400">
            Sized for the tools on our platform — light equity apps up to
            512 GB solver farms. Cancel anytime from your dashboard.
          </p>
        </div>
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {pricingPlans.map((plan) => (
            ...
            >
              ...
              {!loading && user ? (
                <Link
                  href={`/dashboard/checkout/${plan.id}`}
                ...
                <Link
                  href={`/signin?callbackUrl=${encodeURIComponent(`/dashboard/checkout/${plan.id}`)}`}
                  className={`mt-8 block w-full rounded-xl py-3 text-center text-sm font-semibold transition ${
                    plan.highlighted
                      ? "bg-gold-500 text-felt-950 hover:bg-gold-400"
                      : "border border-white/15 text-white hover:bg-white/5"
